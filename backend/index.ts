...
app.get('/api/dashboard', async (req: Request, res: Response) => {
    try {
        ...
        let appointmentWhereClause: any = {};
        ...
        const totalNet = invoices.reduce((sum: number, inv: any) => sum + inv.totalNet, 0);
        const totalGross = invoices.reduce((sum: number, inv: any) => sum + (inv.totalGross || 0), 0);
        const totalDiscount = invoices.reduce((sum: number, inv: any) => sum + (inv.totalDiscount || 0), 0);

        // Total items sold (summing quantities)
        const totalItemsSold = invoices.reduce((sum: number, inv: any) => {
            return sum + (inv.items?.reduce((iSum: number, item: any) => iSum + (item.quantity || 1), 0) || 0);
        }, 0);
        ...
        const uniqueCustomerIds = new Set(invoices.map((inv: any) => inv.customerId).filter(Boolean));
        ...
        const paymentMethods = {
            cash: payments.filter((p: any) => p.method === 'Cash').reduce((s: number, p: any) => s + p.amount, 0),
            card: payments.filter((p: any) => p.method === 'Card').reduce((s: number, p: any) => s + p.amount, 0),
            gpay: payments.filter((p: any) => p.method === 'GPay').reduce((s: number, p: any) => s + p.amount, 0),
            paytm: payments.filter((p: any) => p.method === 'Paytm').reduce((s: number, p: any) => s + p.amount, 0),
            phonepe: payments.filter((p: any) => p.method === 'PhonePe').reduce((s: number, p: any) => s + p.amount, 0),
        };
        ...
        invoices.forEach((inv: any) => {
            inv.items?.forEach((item: any) => {
                ...
                if (name) {
                    if (!serviceRevMap[name]) serviceRevMap[name] = { name, revenue: 0, count: 0 };
        ...
        // Staff performance
        const staffMap: Record<string, { id: string; name: string; revenue: number; bills: number }> = {};
        invoices.forEach((inv: any) => {
            inv.items?.forEach((item: any) => {
                const sid = item.staff?.id;
                const sname = item.staff?.name;
                if (sid && sname) {
                    if (!staffMap[sid]) staffMap[sid] = { id: sid, name: sname, revenue: 0, bills: 0 };
...
// Membership Plans
app.get('/api/membership-plans', async (req: Request, res: Response) => {
    try {
...
app.post('/api/membership-plans', async (req: Request, res: Response) => {
    try {
...
// Inactive Customers
app.get('/api/customers/inactive', async (req: Request, res: Response) => {
    try {
    ...
    try {
        ...
        while (retries > 0) {
            try {
                ...
                const membershipItem = items.find((item: any) => item.membershipPlanId);
...
app.get('/api/billing/:id', async (req: Request, res: Response) => {
    try {
        const id = req.params.id as string;
...
// Update Bill/Invoice
app.put('/api/billing/:id', async (req: Request, res: Response) => {
    const invoiceIdParam = req.params.id as string;
    const { customerId, items, payments, totalDiscount = 0 } = req.body;
...
// Delete Bill/Invoice
app.delete('/api/billing/:id', async (req: Request, res: Response) => {
    try {
        const invoiceIdParam = req.params.id as string;
...
// Day Closing
app.get('/api/closing/status', async (req: Request, res: Response) => {
    try {
        const { date } = req.query;
        ...
        const totalUPI = paymentsDay
            .filter((p: any) => p.method === 'GPay' || p.method === 'Paytm' || p.method === 'PhonePe')
            .reduce((sum: number, p: any) => sum + p.amount, 0);
...
app.post('/api/closing', async (req: Request, res: Response) => {
    try {
